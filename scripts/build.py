...
body += '<section class="prose"><h2>Raise the Skyward Watch</h2><p>The crane swings a course of stone across the wall and one tap sets it. Anything that overhangs the course below crumbles into the gorge and the wall is narrower from then on \u2014 but a course that loses nothing is laid <b>true</b>, the crane sends another stone up, and the next course may oversail the one below. A badly climbed wall can still be won back.</p><p>Thirteen walls across three marches, each with its own width, load and swing table, and each with a crown to reach. The Free Climb has no top at all. Every wall keeps the ten tallest climbs with their replays, so any of the ten can be watched \u2014 nothing sends a score, a climb sends the climb and it is re-run to work the height out.</p></section>'
page('/small-games/tower-stack/', 'Tower Stack', 'Free one-button stacking game: thirteen stone walls, a crown on each, and a public top ten kept with replays.', body, 'small-games', [('Small Games','/small-games/'),('Tower Stack','/small-games/tower-stack/')], '/assets/projects/tower-stack.webp')
